...
namespace ffmpeg
{
	namespace codecs
	{
		...
		video_decoder_qsv::video_decoder_qsv(const video_decoder_options& options)
			: video_decoder(options)
		{
			...
			else if (options.codec_id == AVCodecID::AV_CODEC_ID_VP9)
			{
				codec_name_ = "vp9_qsv";
			}
		}

		video_decoder_qsv::~video_decoder_qsv()
		{
			close();
		}

		bool video_decoder_qsv::open()
		{
		...
		bool video_decoder_qsv::receive_frame(AVFrame* frame)
		{
			AVFrame gpu = {};
