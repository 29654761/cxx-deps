...
namespace ffmpeg
{
	namespace codecs
	{
        video_encoder_vp8_qsv::video_encoder_vp8_qsv(const video_encoder_options& options)
			:video_encoder(options)
		{
            codec_name_ = "vp8_nvenc";
		}

        video_encoder_vp8_qsv::~video_encoder_vp8_qsv()
		{
            close();
		}

		bool video_encoder_vp8_qsv::open()
		{
            if (is_opened())
            {
                return false;
