...
namespace ffmpeg
{
	namespace codecs
	{
        video_encoder_h264_cuda::video_encoder_h264_cuda(const video_encoder_options& options)
			:video_encoder(options)
		{
            codec_name_ = "h264_nvenc";
		}

        video_encoder_h264_cuda::~video_encoder_h264_cuda()
		{
            close();
		}

		bool video_encoder_h264_cuda::open()
		{
            if (is_opened())
            {
                return false;
            ...
            std::string vbv_maxrate = std::to_string(options_.max_bitrate / 1024) + "k";
            std::string vbv_bufsize = std::to_string(options_.max_bitrate * 2 / 1024) + "k";
            av_opt_set(context_->priv_data, "rc", "vbr_hq", 0);
