...
namespace voip
{
	namespace h323
	{
		void h323_util::make_md5_hash(const PString& alias,const PString& password,const H235_TimeStamp& timestam, PMessageDigest5::Code& digest)
		{
			...
			std::wstring walias = sys::string_util::s2w(alias);
			PWCharArray wc_alias(walias.c_str(), walias.size() + 1, false);
			clearToken.IncludeOptionalField(H235_ClearToken::e_generalID);
			clearToken.m_generalID.SetValueRaw(wc_alias); // Use SetValueRaw to make sure trailing NULL is included

			std::wstring wpassword = sys::string_util::s2w(password);
			PWCharArray wc_password(wpassword.c_str(), wpassword.size() + 1, false);
			clearToken.IncludeOptionalField(H235_ClearToken::e_password);
			clearToken.m_password.SetValueRaw(wc_password);
