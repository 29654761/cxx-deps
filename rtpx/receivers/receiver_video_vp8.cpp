...
namespace rtpx
{
	receiver_video_vp8::receiver_video_vp8(asio::io_context& ioc, int32_t ssrc, media_type_t mt, const sdp_format& fmt, spdlogger_ptr log)
	...
	void receiver_video_vp8::check_for_drop()
	{
		...
		while (!sn::ahead_of<uint16_t>(i, end_seq_))
		{
			...
			vp8_header h = { 0 };
			int hsize = 0;
			vp8_header_deserialize(&h, &hsize, pkt->payload(),(int)pkt->payload_size());
