...
namespace rtpx
{
	receiver_video_h265::receiver_video_h265(asio::io_context& ioc, int32_t ssrc, media_type_t mt, const sdp_format& fmt, spdlogger_ptr log)
	...
	void receiver_video_h265::check_for_drop()
	{
		...
		while (!sn::ahead_of<uint16_t>(i, end_seq_))
		{
			...
			if (log_)
			{
				log_->warn("H265 Drop rtp packet seq={}",i)->flush();
			}
			recv_packs_[idx].reset();
