...
namespace rtpx
{
	receiver_audio_aac::receiver_audio_aac(asio::io_context& ioc, int32_t ssrc, media_type_t mt, const sdp_format& fmt, spdlogger_ptr log)
	...
	bool receiver_audio_aac::process_rfc3640_frame(packet_ptr pkt)
	{
		...
		if (payload_size < (size_t)pos + c * 2)
		{
			return false;
		}
		...
		for (uint16_t i = 0; i < c; i++)
		{
			...
			if (payload_size < (size_t)pos + aupos + ausize)
			{
				break;
			}
