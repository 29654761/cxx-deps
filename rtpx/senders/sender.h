...
namespace rtpx
{
	...
	{
	public:
		typedef std::function<void(packet_ptr)> send_rtp_packet_handler;

		sender(uint8_t pt,uint32_t ssrc,media_type_t mt,const sdp_format& fmt, spdlogger_ptr log);
		virtual ~sender();
		...
		void bind_send_rtp_packet_handler(send_rtp_packet_handler sender) { sender_ = sender; }
	protected:
	...
	protected:
		...
		spdlogger_ptr log_;
		send_rtp_packet_handler sender_;
