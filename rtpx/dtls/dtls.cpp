...
namespace rtpx
{
	...
	std::string dtls::write(const uint8_t* data, size_t size)
	{
		...
		if (buf)
		{
			BIO_read(wbio, buf, (int)buf_size);
	...
	std::string dtls::read(const uint8_t* data, size_t size)
	{
		...
		if (buf)
		{
			int r = SSL_read(ssl_, buf, (int)size);
	...
	std::string dtls::accept(const uint8_t* data, size_t size)
	{
		...
		BIO_read(wbio, rsp, (int)rsp_size);
	...
	std::string dtls::flush()
	{
		...
		while (pending>0)
		{
			...
			if (buf)
			{
				int out = BIO_read(wbio, buf, (int)pending);
				if (out > 0)
				{
					ret.append(buf, out);
				}
	...
	bool dtls::encrypt(const std::vector<uint8_t>& plaintext, std::vector<uint8_t>& ciphertext)
	{
#ifdef RTPX_SSL
		SSL_write(ssl_, plaintext.data(), plaintext.size());

		BIO* out_bio = SSL_get_wbio(ssl_);
		int pending = BIO_pending(out_bio);

		ciphertext.resize(pending);
		BIO_read(out_bio, ciphertext.data(), ciphertext.size());
		return true;
#else
		return false;
#endif
	...
	bool dtls::decrypt(const std::vector<uint8_t>& ciphertext, std::vector<uint8_t>& plaintext)
	{
#ifdef RTPX_SSL
		BIO* in_bio = SSL_get_rbio(ssl_);
		BIO_write(in_bio, ciphertext.data(), ciphertext.size());

		plaintext.resize(1500);
		int len = SSL_read(ssl_, plaintext.data(), plaintext.size());
		if (len <= 0) {
			return false;
		}
