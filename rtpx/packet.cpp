...
namespace rtpx {
	...
	{
		return handle_->header;
	}
	rtp_header* packet::mutable_header()
	{
		return handle_->header;
	}
	...
	packet_ptr packet::clone()const
	{
		packet_ptr pkt = std::make_shared<packet>((uint8_t)this->handle_->header->pt,this->handle_->header->ssrc,
			(uint16_t)this->handle_->header->seq,this->handle_->header->ts);

		pkt->set_payload(this->payload(), this->payload_size());
		return pkt;
	}
