...
stun_attribute_xor_mapped_address stun_attribute_xor_mapped_address::create(const sockaddr* addr, const char* tid)
{
	...
	if (addr->sa_family == AF_INET) {
		...
		//attr.m_address[0] = (uint8_t)(((sin->sin_addr.s_addr & 0xFF000000) >> 24) ^ 0x21);
		//attr.m_address[1] = (uint8_t)(((sin->sin_addr.s_addr & 0x00FF0000) >> 16) ^ 0x12);
		//attr.m_address[2] = (uint8_t)(((sin->sin_addr.s_addr & 0x0000FF00) >> 8) ^ 0xA4);
		//attr.m_address[3] = (uint8_t)(((sin->sin_addr.s_addr & 0x000000FF)) ^ 0x42);

		attr.m_address[0] = (uint8_t)(((sin->sin_addr.s_addr & 0x000000FF)) ^ 0x21);
		attr.m_address[1] = (uint8_t)(((sin->sin_addr.s_addr & 0x0000FF00) >> 8) ^ 0x12);
		attr.m_address[2] = (uint8_t)(((sin->sin_addr.s_addr & 0x00FF0000) >> 16)  ^ 0xA4);
		attr.m_address[3] = (uint8_t)(((sin->sin_addr.s_addr & 0xFF000000) >> 24) ^ 0x42);
	}
...
std::string stun_attribute_message_integrity::create_short_turn(const char* data, int data_size,const std::string& password)
{
	...
	hmac_sha1 ms;
	BYTE ret[20] = { 0 };
	ms.make_bytes((const BYTE*)data, data_size, (const BYTE*)password.data(), (int)password.size(), ret);
