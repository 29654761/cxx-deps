...
namespace rtpx {
	...
	public:
		...
		bool negotiate(const sdp_format& fmt_in,std::vector<sdp_format>& fmt_out)const;

		uint32_t get_default_ssrc()const;

		bool has_ssrc(uint32_t ssrc)const;

		void set_msid(const std::string& sid, const std::string& tid);
	...
	public:
		...
		sdp_setup_t setup = sdp_setup_none;

		std::map<uint8_t, sdp_format> rtpmap;
