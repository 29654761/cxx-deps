...
namespace litertp
{
	receiver_ps::receiver_ps(int32_t ssrc, media_type_t mt, const sdp_format& fmt)
	...
	bool receiver_ps::find_a_frame(std::vector<packet_ptr>& pkts)
	{
		...
		while (!sn::ahead_of<uint16_t>(i, end_seq_))
		{
			...
			if (ts == 0)
			{
				ts = pkt->handle_->header->ts;
			}

			if (ts != pkt->handle_->header->ts)
			{
				detected = true;
				break;
			}
	...
	void receiver_ps::combin_frame(const std::vector<packet_ptr>& pkts)
	{
		...
		litertp::mpeg::ps_frame psframe;
		litertp::mpeg::sys_header sys;
		litertp::mpeg::psm2 psm;
		std::vector<litertp::mpeg::pes2> pes_v;
		std::vector<litertp::mpeg::pes2> pes_a;
