...
namespace litertp
{
	receiver::receiver(int32_t ssrc,media_type_t mt, const sdp_format& fmt)
	{
		ssrc_ = ssrc;
		media_type_ = mt;
		format_ = fmt;
	...
	bool receiver::start()
	{
		...
		active_ = true;
		signal_.reset();
		nack_thread_ = std::make_unique<std::thread>(&receiver::run_nack, this);
		return true;
	}
	...
	bool receiver::insert_packet(packet_ptr pkt)
	{
		...
		if (ssrc_ != pkt->handle_->header->ssrc)
		{
			ssrc_ = pkt->handle_->header->ssrc;
		}
		...
		if (sn::ahead_of<uint16_t>(end_seq_, pkt->handle_->header->seq))
		{
			...
			LOGD("take lost seq=%u\n", pkt->handle_->header->seq);
		}
		else
		{
