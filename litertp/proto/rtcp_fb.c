...
int rtcp_fb_serialize(const rtcp_fb* packet, uint8_t* buffer, size_t size)
{
	...
	write_u32(buffer+4,packet->ssrc_sender);
	write_u32(buffer+8,packet->ssrc_media);
...
int rtcp_fb_parse(rtcp_fb* packet, const uint8_t* buffer, size_t size)
{
	...
	packet->ssrc_sender = read_u32(buffer + 4);
	packet->ssrc_media = read_u32(buffer + 8);
