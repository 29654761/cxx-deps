...
namespace litertp {
	...
	bool transport_custom::send_rtp_packet(packet_ptr packet,const sockaddr* addr,int addr_size)
	{
		...
		send_event_.invoke(port_, 0, (const uint8_t*)b.data(), (int)b.size());
		return true;
	}
	...
	bool transport_custom::send_raw_packet(const uint8_t* raw_data, int size, const sockaddr* addr, int addr_size)
	{
		send_event_.invoke(port_, 0, raw_data, size);
		return true;
	}
