...
int64_t base_message::content_length()const
{
	std::string value;
...
std::string base_message::content_type()const
{
	std::string value;
...
void base_message::set_content_type(const std::string& content_type)
{
	headers.set("Content-Type", content_type);
}

bool base_message::set_first_line(const std::string& line)
{
...
bool base_message::set_header_line(const std::string& line)
{
...
void base_message::set_body(const std::string& body)
{
	this->body = body;
...
bool base_message::parse(const std::string& data)
{
	reset();
...
/*
int64_t base_message::parse(const std::string& data, size_t offset)
{
...
*/
int64_t base_message::parse(const char* data, size_t size)
{
	size_t offset = 0;
	...
	int64_t hdclen = 0;
	base_headers::parse_content_length(data, size, hdclen);
...
std::string base_message::to_string()const
{
	std::stringstream ss;
...
bool base_message::is_request()const
{
	return filed2.find_first_of('/', 0) != std::string::npos;
}

bool base_message::match_method(const std::string& method)const
{
